...
#define BUF_SIZE 44//최종길이temp_right와 temp_left와 공백과 ins와 널문자길이 에서 19+21+3=43<44 -> 됨
...
void Init(void* const p, char flag)
{
	if (flag) {
		((p_gyc)p)->m_len = 1;
		printf("%p : (gycchang*)p, %p : &((gycchang*)p)->m_len)\n", (p_gyc)p, &((gycchang*)p)->m_len);//m_len 요소 초기화 후 구조체 변수 시작 주소와 이 변수 주소를 출력해봄
		((p_gyc)p)->m_max_len = 1;
		((p_gyc)p)->m_count = 0;
		((p_gyc)p)->message = NULL;
		((p_gyc)p)->message = (char*)malloc(1);
		strcpy(((p_gyc)p)->message, "a");
	}
}
...
char* method1(const char(*ap_str)[BUF_SIZE])
{
	...
	printf("%s : p_tmp_array : method1 반환값\n", p_tmp_array);
	return p_tmp_array;
}
void method3(const char(*ap_str)[BUF_SIZE])
{
	char* p_tmp_array = (char*)malloc(BUF_SIZE);
	char tmp_array[BUF_SIZE] = { 0, };
	//strcpy_s(tmp_array, BUF_SIZE, *ap_str);
	strcpy(tmp_array, *ap_str);
	printf("%s : method3 는 ap_str이 가리키는 주소에 저장된 1차원 배열을 바꾼다\n", *ap_str);
}
...
char* SetString_ori(struct gycchang* p_gyc, const char* ap_message)
{
	...
	char temp_right[22];//글자수(21)+1(널문자) = second(s) has passed.로 21글자
	...
	/*수정수정*/
	//method1사용한 부분 제거 -> method3 사용한 걸로 대체
	//char* r_ori = NULL;
	//r_ori =method1(&tmp);
	//printf("%s : r_ori\n", r_ori);
	...
	strcpy(p_gyc->message, ap_message);
	printf("%s : p_gyc->message 원본 문자열의 왼쪽을 모두 받고 난 뒤\n", p_gyc->message);
	/*수정수정*/
	/*char* p = ins;
	p_gyc->m_count++;
	...
	//p = IntToBuf(p_gyc->m_count);
	p_gyc->m_count++;
	memcpy(ins, IntToBuf(p_gyc->m_count), 6);
	//printf("%s: p\n", p);
	//strcpy(ins, p);
	//printf("%s: p, %s: ins\n", p, ins);
	printf("%s : ins\n", ins);
	...
	/*수정수정*/
	printf("%s : temp_right 초기화시 사용할 p_gyc->message\n", p_gyc->message);
	printf("%d : p_gyc->m_max_len : temp_right초기화시 종결조건 인덱스에 사용\n", p_gyc->m_max_len);
	printf("temp_right에러난 이유가 p_gyc->m_max_len이 문자열 길이 합친 것이 아니였을 수도 있다는 생각이 들었다\n");
	printf("따라서 문자열 길이를 합친 것인 strlen(ori)-strlen(temp_left)+1로 종결조건 정함\n");
	printf("%ld:strlen(temp_left) %ld:strlen(ori)\n", strlen(temp_left), strlen(ori));
	printf("temp_right의 선언시 배열크기 다시 계산 : 위의 결과에서 39-18+1 = 22이므로 배열크기 나두기\n");
	/*여기까지*/
	/*수정수정*/
	for (int i = 18; i < p_gyc->m_max_len + 1; i++) {//p_gyc->m_max_len:40//39-18=21번//22번째에 즉, 인덱스21에 널문자 넣어준다
		*(temp_right + i - 18) = *(p_gyc->message + i);
		if (i == p_gyc->m_max_len) {
			*(temp_right + i - 18) = '\0';
			printf("널문자 넣었습니다\n");
		}
	}
	...
	total_size = strlen(temp_left) + strlen(temp_right) + 3;
	printf("%ld : strlen(temp_right), %ld : strlen(temp_left)\n", strlen(temp_left), strlen(temp_right));
	...
	/*수정수정*/
	//memcpy(m_message, temp_left, strlen(temp_left)+1);
	char* p_message = (char*)malloc(strlen(temp_left) + 1);
	//strcpy_s(p_message, strlen(temp_left)+1, temp_left);
	p_message = temp_left;
	printf("%s : p_message\n", p_message);
	//strcpy_s(m_message, strlen(temp_left)+1, temp_left);
	//printf("%s : 최종 반환하려는 값만 출력\n", m_message);

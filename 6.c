...
#include<string.h>//strcat, memcpy, strcat_s, strlen, strcpy_s, strcpy
...
char* SetString(struct gycchang* p_gyc, const char* ap_str)
{
	...
	//strcpy_s(tmp_array, MAX_LEN, r_tmp);삭제
	...
	if (message == NULL) {//처음할당
		...
		//strcpy_s(message, total_size, temp_left);삭제
		strcpy(message, temp_left);
	}
	else {//처음할당아닌 경우
		total_size = strlen(temp_left) + 1;
		message = (char*)realloc(message, total_size);
		//strcat_s(message, total_size, temp_left);삭제
		strcat(message, temp_left);
	}
	total_size = p_gyc->m_max_len + 5;
	message = (char*)realloc(message, total_size);
	//strcat_s(message, total_size, ins);//숫자 들어갈 자리(1)+나머지 문자열 자리수(m_max_len,널문자포함)
	//strcat_s(message, total_size, " ");
	//strcat_s(message, total_size, temp_right);//p_gyc->m_max_len : 39, temp_left : 20 + temp_right : 22 = 42 -> 39+3
	strcat(message, ins);//숫자 들어갈 자리(1)+나머지 문자열 자리수(m_max_len,널문자포함)
	strcat(message, " ");
	strcat(message, temp_right);//p_gyc->m_max_len : 39, temp_left : 20 + temp_right : 22 = 42 -> 39+3
